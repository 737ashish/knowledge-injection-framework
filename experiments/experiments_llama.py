...
def llama_formatting(prompt, tokenizer):
    messages = [
...
def llama_formatting_injection(prompt, tokenizer):
    messages = [
...
def extract_prompt(text):    
    match = re.search(r"Prompt:\s*(.*?)<\|eot_id\|>", text, re.DOTALL)    
    if match:
...
for n, row in df.iterrows():
    ...
    prompt = llama_formatting(row["prompt"], tokenizer)
    gen = extract_answer(generate(prompt, tokenizer, model))
    ...
    if method == "in-context":
        ...
        injection_prompt = llama_formatting_injection(injection_prompt, tokenizer)
        gen_injection = generate(injection_prompt, tokenizer, model)
        prompt = extract_prompt(injection_prompt)
        gen_injection = extract_answer(gen_injection)
        t_end = time.time()
