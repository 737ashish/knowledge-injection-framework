...
def qwen_formatting(prompt, tokenizer):
    messages = [
...
def qwen_formatting_injection(prompt, tokenizer):
    messages = [
    {"role": "system", "content": "You are given new information. Complete the given prompt using the new information."},
    {"role": "user", "content": prompt}
    ]
...
for n, row in df.iterrows():
    ...
    prompt = qwen_formatting(row["prompt"], tokenizer)
    gen = generate(prompt, tokenizer, model).replace("system\nYou are Qwen, created by Alibaba Cloud. You are a helpful assistant.\nuser\n", "").replace("\nassistant\n", " ")
    ...
    if method == "in-context":
        ...
        injection_prompt = qwen_formatting_injection(injection_prompt, tokenizer)
        gen_injection = generate(injection_prompt, tokenizer, model)
        prompt = injection_prompt.split(hparams.split_token)[-1].strip().split(hparams.split_token)[-1].strip().replace("<|im_end|>\n<|im_start|>assistant", " ").strip()
        gen_injection = gen_injection[gen_injection.find(prompt):].replace("\nassistant\n", " ").strip()
        t_end = time.time()
